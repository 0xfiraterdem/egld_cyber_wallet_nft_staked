...
for wallet in url:
    if wallet['sender'] not in stake_wallet:
        ...
        data1 = requests.post(post_url,
                              json={"scAddress": "erd1qqqqqqqqqqqqqpgqsu2vxxx5l3tjgcnjl6mftlz5dtz5cp5s398syqw3gz","funcName": "getUserStakedNftNonces",
                                    "args": [f"{hex_wallet}"]})
            ...
            if 'returnData' in data1.json().keys():
                ...
                if 'returnData' in data2.json().keys():
                    ...
                    if 'returnData' not in data1.json().keys():
                        ...
                        balance_unbonding.extend(wallet_balance_nft.wallet_balance(wallet['sender']))
